...
# Upload alignment
alig <- read.csv(paste(work_dir, '/', ucsc_id, '_genome_dir/', ucsc_id, '_mapping.txt'), 
...
for (i in c(1:nrow(chr_info))){
  ...
  chr_vis <- ggplot(chr_density) +
    ...
    scale_fill_gradientn(
      ...
      limits = c(0, 1)) +
    annotate("rect", xmin = chr_info$centStart[i], xmax = chr_info$centEnd[i], ymin = 0, ymax = 1,
             alpha = 1, fill = "black") +
    labs(title = paste('chromosome', str_split_i(chr_info$chrom[i], 'chr', -1), '\n', 'N =', nrow(alig_chr))) +
    theme_bw()
  
  ggsave(paste(work_dir, '/plots/', ucsc_id, "_coloring_", chr_info$chrom[i], ".pdf", sep = ''),
...
main_plot <- wrap_plots(plot_list, ncol = 11)
